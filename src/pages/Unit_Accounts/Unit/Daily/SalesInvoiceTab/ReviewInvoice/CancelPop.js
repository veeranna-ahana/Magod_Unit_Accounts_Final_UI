...
export default function CancelPop({
  popupBox,
  setpopBox,
  selectValues,
  cancelData,
  ...
  const handlePostData = async () => {
    ...
    try {
      const response = await axios.post(
        baseURL + `/dailyReport/cancelInvoiceVoucher`,
        { data: data }
      );
      ...
      <div>
        <Modal show={popupBox} onHide={handleClose}>
          ...
          <Modal.Body style={{ fontSize: "12px" }}>
            Do you wish to Cancel Invoice No {selectValues.Inv_No} Dt{" "}
            {selectValues.date} and revert stock ?
          </Modal.Body>
